...
실행 메모(2026-08-16): `20260816_grid_origin_sensitivity_v1`에서 공식 NetCDF를 EPSG:6933의 5·10·20 km 격자로 다시 집계하고, 각 크기마다 기본·x 반셀·y 반셀·x/y 반셀의 네 원점을 평가했다. 주 비교는 10 km의 500-pixel footprint 기준을 셀 면적에 맞춰 5 km=125, 10 km=500, 20 km=2,000 pixels로 조정했으며, literal 500-pixel 기준은 코호트 선택 민감도로 분리했다. 면적비례 12개 사양 모두에서 current-only AP lift의 2년 block-bootstrap 95% CI가 0보다 컸다. 같은 크기 내 원점별 lift 최대 범위는 0.0318이었지만, 10 km 기본 격자 대비 픽셀 가중 Top-20% 조사공간 Jaccard 최솟값은 0.5073이었다. 따라서 현재상태 위험순위 신호의 성능 강건성과 실제 우선조사 위치의 안정성을 서로 다른 결론으로 보고한다. 궤적 증분은 8/12 사양, OISST 증분은 0/12 사양에서만 CI 하한이 0보다 컸다.
...
실행 메모(2026-08-16, E2E-v2-E20B): 3월 31일 pre-season clock에서 월평균 실패가 일별 평균화 때문인지 확인하기 위해 기존 E20의 165셀·164개 최근접 CRW 픽셀과 2,478개 Q2–Q3 평가행을 그대로 고정했다. 결과를 보기 전에 `7일 이동평균 anomaly 최대`, `양의 anomaly degree-days`, `1985–2004 지역 Q1 p90 초과일수`, `최대 연속 고온일수` 네 특징만 등록했다. 1985–2024년 Q1 592,040 픽셀-일을 추출했고 예측기간 특징 완전성은 100%, 2021-01-15 NOAA STAR 원본과 PaciOOS ERDDAP 미러의 164픽셀 최대 절대차는 0.000°C였다. Logistic의 일별 블록 대 궤적 평균 AP lift 차이는 -0.00183(95% CI -0.01627–0.00930), 월별 CRW 이후 일별 순수 추가분은 -0.00005(-0.02479–0.01851)였다. Random Forest와 XGBoost에서도 양의 증분이 지지된 비교는 없었고 독립 재계산 19/19가 통과했다. 따라서 시나리오 D에 따라 `시험한 Q1 공개 5 km SST의 평균·극한·지속시간 표현은 canopy history 이후 안정적 증분 예측가치를 보이지 않았다`까지만 허용한다. 열스트레스의 생태적 무관성이나 다른 시기·공간 support의 무효를 주장하지 않는다.
